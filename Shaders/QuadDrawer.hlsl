...
float4 QuadRGBAPS( VS_OUTPUT Input ) : SV_TARGET
{
	...
	res.xyz *= g_Scale;
	res.x = pow(res.x, 1.0f / 2.2f);
	res.y = pow(res.y, 1.0f / 2.2f);
	res.z = pow(res.z, 1.0f / 2.2f);
...
float4 QuadRGBANORMAL(VS_OUTPUT Input) : SV_TARGET
{
	float4 res = inputTexture2.Sample(g_PointSampler, Input.vTexcoord);
...
float4 QuadRGBALINEAR(VS_OUTPUT Input) : SV_TARGET
{
	float4 res = inputTexture2.Sample(g_PointSampler, Input.vTexcoord);
