...
void CUDAMarchingCubesHashSDF::copyNoTextureTrianglesToCPU() {
	...
	if (!GlobalAppState::get().s_offlineProcessing) {
		if (nTriangles != 0) {
			...
			for (unsigned int i = 0; i < 3 * nTriangles; i++) {
				...
				m_meshData.m_Colors[baseIdx + i] = vec4f(powf(vc[3 * i + 1].x, 1.f / 2.2f), powf(vc[3 * i + 1].y, 1.f / 2.2f), powf(vc[3 * i + 1].z, 1.f / 2.2f), 1.0f);
			}
		}
	}
	else {
		...
		if (nTriangles != 0) {
			...
			for (unsigned int i = 0; i < 3 * nTriangles; i++) {
				...
				md.m_Colors[i] = vec4f(powf(vc[3 * i + 1].x, 1.f / 2.2f), powf(vc[3 * i + 1].y, 1.f / 2.2f), powf(vc[3 * i + 1].z, 1.f / 2.2f), 1.0f);
			}
...
void CUDAMarchingCubesHashSDF::copyTrianglesToCPU(TexPoolData texPoolData, TexPoolParams texPoolParams, float *d_lightData) {
	...
	if (!GlobalAppState::get().s_offlineProcessing) {
		...
		if (nTriangles != 0) {
			...
			uchar *h_normalTextureImg = (uchar *)malloc(sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);

			uchar *d_textureImg, *d_normalTextureImg;
			...
			cutilSafeCall(cudaMalloc(&d_normalTextureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth));
			cudaMemset(d_normalTextureImg, 0, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			...
			free(h_textureImg);
			free(h_normalTextureImg);
			cutilSafeCall(cudaFree(d_textureImg));
			cutilSafeCall(cudaFree(d_normalTextureImg));
		...
		if (nTriangles != 0) {
			...
			for (unsigned int i = 0; i < 3 * nTriangles; i++) {
				...
				//std::cout << md.m_TextureCoords[i] << std::endl;
			}
			printf("Texcoord indexing finish\n");
			...
			uchar *h_textureImg = (uchar *)malloc(sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			uchar *h_albedoTextureImg = (uchar *)malloc(sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			uchar *h_normalTextureImg = (uchar *)malloc(sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			float *h_lightData = (float *)malloc(sizeof(float) * 9);

			uchar *d_textureImg, *d_normalTextureImg, *d_albedoTextureImg;
			unsigned int *d_textureAddress;
			cutilSafeCall(cudaMalloc(&d_textureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth));
			cudaMemset(d_textureImg, 0, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			cutilSafeCall(cudaMalloc(&d_albedoTextureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth));
			cudaMemset(d_albedoTextureImg, 0, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			cutilSafeCall(cudaMalloc(&d_normalTextureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth));
			cudaMemset(d_normalTextureImg, 0, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth);
			cutilSafeCall(cudaMalloc(&d_textureAddress, sizeof(unsigned int) * textureTileWidth * textureTileWidth));
			cudaMemset(d_textureAddress, 0, sizeof(unsigned int) * textureTileWidth * textureTileWidth);
			memset(h_lightData, 0, sizeof(float) * 9);

			cudaMemcpy(d_textureAddress, uniqueTextureIndex.data(), sizeof(unsigned int) * h_numTextureTile, cudaMemcpyHostToDevice);
			//exportTexture(d_textureImg, d_textureAddress, texPoolData.d_texPatches, textureTileWidth, textureTileWidth, texPoolParams.m_texturePatchWidth, texPoolParams.m_texturePatchSize, h_numTextureTile, texPoolParams.m_numTexturePatches);
			if (GlobalAppState::get().s_geometryOptimization) {
				exportDoubleTexture(d_textureImg, d_albedoTextureImg, d_normalTextureImg, d_textureAddress, texPoolData.d_texPatches, textureTileWidth, textureTileWidth, texPoolParams.m_texturePatchWidth, texPoolParams.m_texturePatchSize, h_numTextureTile, texPoolParams.m_numTexturePatches, d_lightData);
				cudaMemcpy(h_lightData, d_lightData, sizeof(float) * 9, cudaMemcpyDeviceToHost);
			}
			else {
				exportTexture(d_textureImg, d_textureAddress, texPoolData.d_texPatches, textureTileWidth, textureTileWidth, texPoolParams.m_texturePatchWidth, texPoolParams.m_texturePatchSize, h_numTextureTile, texPoolParams.m_numTexturePatches);
			}
			cudaMemcpy(h_textureImg, d_textureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth, cudaMemcpyDeviceToHost);
			cudaMemcpy(h_albedoTextureImg, d_albedoTextureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth, cudaMemcpyDeviceToHost);
			cudaMemcpy(h_normalTextureImg, d_normalTextureImg, sizeof(uchar) * 3 * (texPoolParams.m_texturePatchWidth + 2) * (texPoolParams.m_texturePatchWidth + 2) * textureTileWidth * textureTileWidth, cudaMemcpyDeviceToHost);
			...
			std::string normalTextureFileName = GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_texture_normal.png";
			std::string albedoTextureFileName = GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_texture_albedo.png";
			std::string lightTxtFileName = GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_light.txt";
			...
			std::string folderNormalTextureFileName = folderName + GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_texture_normal.png";
			std::string folderAlbedoTextureFileName = folderName + GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_texture_albedo.png";
			std::string folderLightTxtFileName = folderName + GlobalAppState::get().s_sceneName + GlobalAppState::get().export_surfix[GlobalAppState::get().s_optimizationIdx] + "_light.txt";
			...
			while (util::fileExists(folderTextureFileName)) {
				...
				folderNormalTextureFileName = path + base + "_normal" + std::to_string(num + 1) + ".png";
				normalTextureFileName = base + "_normal" + std::to_string(num + 1) + ".png";
				folderAlbedoTextureFileName = path + base + "_albedo" + std::to_string(num + 1) + ".png";
				albedoTextureFileName = base + "_albedo" + std::to_string(num + 1) + ".png";
				folderLightTxtFileName = path + base + "_light" + std::to_string(num + 1) + ".txt";
				lightTxtFileName = base + "_light" + std::to_string(num + 1) + ".txt";
			}
			...
			cv::Mat albedoTextureMat(textureTileWidth * (texPoolParams.m_texturePatchWidth + 2), textureTileWidth * (texPoolParams.m_texturePatchWidth + 2), CV_8UC3, h_albedoTextureImg);
			cv::cvtColor(albedoTextureMat, albedoTextureMat, CV_RGB2BGR);
			cv::imwrite(folderAlbedoTextureFileName, albedoTextureMat);

			cv::Mat normalTextureMat(textureTileWidth * (texPoolParams.m_texturePatchWidth + 2), textureTileWidth * (texPoolParams.m_texturePatchWidth + 2), CV_8UC3, h_normalTextureImg);
			cv::imwrite(folderNormalTextureFileName, normalTextureMat);

			if (GlobalAppState::get().s_geometryOptimization) {
				std::ofstream outLightFile(folderLightTxtFileName, std::ofstream::out);
				for (int i = 0; i < 9; i++) {
					outLightFile << h_lightData[i] << std::endl;
				}
				outLightFile.close();
			}
			...
			cutilSafeCall(cudaFree(d_textureImg));
			cutilSafeCall(cudaFree(d_normalTextureImg));
			cutilSafeCall(cudaFree(d_albedoTextureImg));
			cutilSafeCall(cudaFree(d_textureAddress));
			free(h_textureImg);
			free(h_normalTextureImg);
			free(h_albedoTextureImg);
		}
	}
...
void CUDAMarchingCubesHashSDF::saveMeshWithoutTexture(const std::string& filename, const mat4f *transform /*= NULL*/, bool overwriteExistingFile /*= false*/)
{
	std::string folder = util::directoryFromPath(filename);
	if (!util::directoryExists(folder)) {
		util::makeDirectory(folder);
		...
		for (int y = minGridPos.y; y < maxGridPos.y; y++) {
			for (int z = minGridPos.z; z < maxGridPos.z; z++) {
				...
				if (chunkGrid.containsSDFBlocksChunk(chunk)) {
					...
					extractIsoSurface(chunkGrid.getHashData(), chunkGrid.getHashParams(), rayCastData, texPoolData, texPoolParams, NULL, minCorner, maxCorner, true);
...
void CUDAMarchingCubesHashSDF::extractIsoSurface(const HashData& hashData, const HashParams& hashParams, const RayCastData& rayCastData, const TexPoolData& texPoolData, const TexPoolParams& texPoolParams, float* d_lightData, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled)
{
	resetMarchingCubesCUDA(m_data);
	...
	copyTrianglesToCPU(texPoolData, texPoolParams, d_lightData);
}
...
void CUDAMarchingCubesHashSDF::extractIsoSurface(const HashData& hashData, const HashParams& hashParams, const RayCastData& rayCastData, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled)
{
	...
	extractIsoSurfacePass1CUDA(hashData, rayCastData, m_params, m_data);
	extractIsoSurfacePass2WithoutTextureCUDA(hashData, rayCastData, m_params, m_data, m_data.getNumOccupiedBlocks());

	std::cout << "Isosurface finish\n" << std::endl;
	copyNoTextureTrianglesToCPU();
}
