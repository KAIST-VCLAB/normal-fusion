...
__global__ void renderCorrespondenceDepthLocal2_kernel(unsigned int imageWidth, unsigned int imageHeight, float4 *output, float4 *d_targetIntensity, CameraTrackingLocalInput cameraTrackingInput, float3x3 intrinsics, CameraTrackingLocalParameters cameraTrackingParameters, float3 *d_x_rot, float3 *d_x_trans) {
	...
	if (index.x < imageWidth && index.y < imageHeight) {
		...
		if (pInput.x != MINF && !nInput.x != MINF)
		{
			...
			if (pProjTrans.z > 0.f) {
				...
				float4 iTarget = bilinearInterpolationFloat4(uvModel.x, uvModel.y, d_targetIntensity, imageWidth, imageHeight);
...
extern "C" void renderCorrespondenceDepthLocal2CUDA(unsigned int imageWidth, unsigned int imageHeight, float4 *output, float4* d_targetIntensity, CameraTrackingLocalInput cameraTrackingInput, float* intrinsics, CameraTrackingLocalParameters cameraTrackingIParameters, float3* d_x_rot, float3* d_x_trans) {
	...
	renderCorrespondenceDepthLocal2_kernel << < grid, block >> > (imageWidth, imageHeight, output, d_targetIntensity, cameraTrackingInput, float3x3(intrinsics), cameraTrackingIParameters, d_x_rot, d_x_trans);
