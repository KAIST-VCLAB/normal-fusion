...
CUDACameraTrackingMultiResLocalDepth* g_cameraTrackingLocalDepth = NULL;
CUDACameraTrackingMultiResLocalRGB* g_cameraTrackingGeometryLocalRGB = NULL;
...
CUDAInputEnhancement*		g_inputEnhance = NULL;
...
void StopScanningAndExtractIsoSurfaceMC(const std::string& filename, bool overwriteExistingFile /*= false*/)
{
	...
	if (!GlobalAppState::get().s_streamingEnabled) {
		...
		if (!GlobalAppState::get().s_voxelRenderingEnabled) g_marchingCubesHashSDF->extractIsoSurface(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_rayCast->getRayCastData(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData().d_lightData);
		else g_marchingCubesHashSDF->extractIsoSurface(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_rayCast->getRayCastData()); //g_chunkGrid->startMultiThreading();
	}
	else {
	...
	if (!GlobalAppState::get().s_voxelRenderingEnabled) g_marchingCubesHashSDF->saveMesh(filename, NULL, overwriteExistingFile);
	else g_marchingCubesHashSDF->saveMeshWithoutTexture(filename, NULL, overwriteExistingFile);
...
void CALLBACK OnKeyboard(UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext)
{
	...
	if (bKeyDown) {
		...
		case 'K':
			...
			break;
		case 'L':
			GlobalAppState::get().s_RenderMode = 15;
			break;
		case 'S':
...
HRESULT CALLBACK OnD3D11CreateDevice(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	...
	g_CudaDepthSensor.setBlurScore(GlobalAppState::get().s_numKeyScoreCounter, GlobalAppState::get().s_numPassScoreCounter);
	...
	//added by HHHA
	g_cameraTrackingLocalDepth = new CUDACameraTrackingMultiResLocalDepth(g_RGBDAdapter.getWidth(), g_RGBDAdapter.getHeight(), GlobalWarpingState::get().s_maxLevels, GlobalWarpingState::get().s_offsetx, GlobalWarpingState::get().s_offsety, GlobalWarpingState::get().s_cellWidth, GlobalWarpingState::get().s_cellHeight, GlobalWarpingState::get().s_localWindowHWidth);

	std::vector<float> newCellWidth, newCellHeight;
	std::vector<int> newLocalWindowWH;
	newCellWidth.clear();
	newCellHeight.clear();
	newLocalWindowWH.clear();
	float resM = 1.f;
	for (int i = 0; i < GlobalWarpingState::get().s_cellHeight.size(); i++) {
		newCellWidth.push_back(GlobalWarpingState::get().s_cellWidth[i] * resM);
		newCellHeight.push_back(GlobalWarpingState::get().s_cellHeight[i] * resM);
		newLocalWindowWH.push_back(GlobalWarpingState::get().s_localWindowHWidth[i] * resM);
	}
	g_cameraTrackingGeometryLocalRGB = new CUDACameraTrackingMultiResLocalRGB(g_RGBDAdapter.getWidth(), g_RGBDAdapter.getHeight(), 1, GlobalWarpingState::get().s_offsetx, GlobalWarpingState::get().s_offsety, newCellWidth, newCellHeight, newLocalWindowWH);

	g_sceneRep = new CUDASceneRepHashSDF(CUDASceneRepHashSDF::parametersFromGlobalAppState(GlobalAppState::get()));

	//added by Joo Ho Lee
	if (!GlobalAppState::get().s_voxelRenderingEnabled) g_texUpdate = new CUDATexUpdate(CUDATexUpdate::parametersFromGlobalAppState(GlobalAppState::get()), CUDATexUpdate::texPoolParametersFromGlobalAppState(GlobalAppState::get()), CUDATexUpdate::rayCastParametersFromGlobalAppState(GlobalAppState::get()));

	//added by Hyunho Ha
	g_inputEnhance = new CUDAInputEnhancement(CUDAInputEnhancement::parametersFromGlobalState(GlobalAppState::get(), GlobalEnhancementState::get(), g_RGBDAdapter.getColorIntrinsics()));
	...
	if (!GlobalAppState::get().s_voxelRenderingEnabled)  g_texUpdate->bindModelTextures(g_rayCast->getRayCastData());
...
void CALLBACK OnD3D11DestroyDevice(void* pUserContext)
{
	...
	SAFE_DELETE(g_cameraTrackingLocalRGB);
	SAFE_DELETE(g_cameraTrackingLocalDepth);
	SAFE_DELETE(g_cameraTrackingGeometryLocalRGB);
	...
	SAFE_DELETE(g_inputEnhance);
	SAFE_DELETE(g_texUpdate);
...
void reconstruction(ID3D11DeviceContext* pd3dImmediateContext)
{
	...
	if (g_RGBDAdapter.getFrameNumber() > 1) {
		...
		if (GlobalAppState::get().s_voxelRenderingEnabled)
			...
			g_rayCast->doubleTexRender(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData(), renderTransform);
		...
		else
		{
			...
			if (!GlobalAppState::get().s_trackingEnabled) {
				...
				if (!useRGBDTracking) {
					...
					transformation = transformationrgb;
				}
			}
		}
		...
		if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
			...
			TimingLog::totalTimeICP += GlobalAppState::get().s_Timer.getElapsedTimeMS(); TimingLog::countTimeICP++;
	...
	if (!GlobalAppState::get().s_voxelRenderingEnabled) g_texUpdate->updateNumFrame();

	///////////////////////////////////////////////////////////////////////////
	// Integration begin when RGB image pass blur score                      //
	///////////////////////////////////////////////////////////////////////////

	if (GlobalAppState::get().s_integrationEnabled && g_CudaDepthSensor.passBlurScore(g_RGBDAdapter.getFrameNumber())) {
		///////////////////////////////////////////////////////////////////////////
		// Input Enhancement                                                     //
		// Estimate light, albedo, and refine depth					             //
		///////////////////////////////////////////////////////////////////////////

		if (GlobalAppState::get().s_timingsDetailledEnabledOurs) { GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.start(); }

		if (g_RGBDAdapter.getFrameNumber() > 1) {
			if (!GlobalAppState::get().s_voxelRenderingEnabled) g_rayCast->doubleTexRender(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData(), transformation);
			else g_rayCast->render(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_CudaDepthSensor.getDepthCameraData(), transformation);
		}
		else g_rayCast->setLastViewMatrix(transformation);

		g_inputEnhance->enhanceInput(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_CudaDepthSensor.getDepthCameraData(), g_CudaDepthSensor.getDepthCameraParams(), g_rayCast->getRayCastData(), g_rayCast->getRayCastParams(), g_texUpdate->getTexUpdateData(),
			GlobalEnhancementState::getInstance().s_maxOuterIter,
			GlobalEnhancementState::getInstance().s_maxInnerIter,
			GlobalEnhancementState::getInstance().s_weightsDataShading,
			GlobalEnhancementState::getInstance().s_weightsDepthTemp,
			GlobalEnhancementState::getInstance().s_weightsDepthSmooth,
			GlobalEnhancementState::getInstance().s_weightsAlbedoTemp,
			GlobalEnhancementState::getInstance().s_weightsAlbedoSmooth,
			GlobalEnhancementState::getInstance().s_weightsLightTemp,
			(g_RGBDAdapter.getFrameNumber() <= 2) | GlobalAppState::get().s_voxelRenderingEnabled,
			g_RGBDAdapter.getFrameNumber());
		if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
			GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.stop();
			TimingLog::totalTimeEnhanceInput += GlobalAppState::get().s_Timer.getElapsedTimeMS(); TimingLog::countTimeEnhanceInput++;
			printf("input enhancement: %lf\n", GlobalAppState::get().s_Timer.getElapsedTimeMS());
		}
		...
		g_sceneRep->integrate(transformation, g_CudaDepthSensor.getDepthCameraData(), g_CudaDepthSensor.getDepthCameraParams(), g_chunkGrid->getBitMaskGPU(), g_CudaDepthSensor.getDepthCameraData().d_depthMask);
		...
		if (!GlobalAppState::get().s_voxelRenderingEnabled) {
			// Resize the hashBucketNum, or voxel size!
			...
			if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
				GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.stop();
				TimingLog::totalTimeTransferTexture += GlobalAppState::get().s_Timer.getElapsedTimeMS(); TimingLog::countTimeTransferTexture++;
				printf("texUpdateFromPreviousTexture: %lf\n", GlobalAppState::get().s_Timer.getElapsedTimeMS());
			}
			...
			if (g_RGBDAdapter.getFrameNumber() > 1 && g_texUpdate->getNumNonUpdatedFrame() >= GlobalAppState::get().s_texKeyFrames) {
				if (g_RGBDAdapter.getFrameNumber() > 3) { //GlobalAppState::getInstance().s_texKeyFrames * 2 - 1) {
					...
					g_rayCast->doubleTexRender(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData(), transformation);
					...
					if (GlobalAppState::get().s_optimizationIdx == GlobalAppState::get().Optim_Ours) {
						...
						///////////////////////////////////////////////////////////////////////////
						{
							g_cameraTrackingLocalRGB->applyMovingDLTWithNormalsOurs(
								g_rayCast->getRayCastData().d_depth4, g_rayCast->getRayCastData().d_normals, g_rayCast->getRayCastData().d_normals, g_rayCast->getRayCastData().d_colors,
								g_texUpdate->getSourceMask(),
								g_CudaDepthSensor.getDepthCameraData().d_normalData, g_CudaDepthSensor.getDepthCameraData().d_colorData,
								...
								g_RGBDAdapter.getDepthIntrinsics(), g_CudaDepthSensor.getDepthCameraData(),
								GlobalWarpingState::getInstance().s_lambdaNWarp, GlobalWarpingState::getInstance().s_lambdaCWarp);

							if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
								GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.stop();
								TimingLog::totalTimeOptimizeTexture += GlobalAppState::get().s_Timer.getElapsedTimeMS(); TimingLog::countTimeOptimizeTexture++;
								printf("applyMovingDLTOurs: %lf\n", GlobalAppState::get().s_Timer.getElapsedTimeMS());
							}
							...
							if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
								GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.stop();
								TimingLog::totalTimeBlendingTexture += GlobalAppState::get().s_Timer.getElapsedTimeMS(); TimingLog::countTimeBlendingTexture++;
								printf("textureBlending: %lf\n", GlobalAppState::get().s_Timer.getElapsedTimeMS());
							}
						}
					}
					else {
						...
						g_rayCast->doubleTexRender(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData(), transformationrgb);
		...
		else g_sceneRep->finalizeIntegration(g_CudaDepthSensor.getDepthCameraData());
	}
...
void CALLBACK OnD3D11FrameRender(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext)
{
	...
	if (g_RGBDAdapter.getFrameNumber() == GlobalAppState::get().s_nVideoFrame && GlobalAppState::get().s_reconstructionEnabled && GlobalAppState::get().s_sensorIdx == 9) {
		GlobalAppState::get().WaitForGPU(); GlobalAppState::get().s_Timer.stop();
	...
	if (GlobalAppState::get().s_RenderMode == 0) {
		...
		if (GlobalAppState::get().s_optimizationIdx == 0 && GlobalAppState::get().s_geometryOptimization) {
			DX11PhongLighting::render(DXUTGetD3D11Device(), pd3dImmediateContext, g_CustomRenderTarget.GetSRV(1), (float *)g_rayCast->getRayCastData().d_detailNormals, g_CustomRenderTarget.GetSRV(3), GlobalAppState::get().s_useColorForRendering, g_CustomRenderTarget.getWidth(), g_CustomRenderTarget.getHeight());
		}
		else {
	...
	else if (GlobalAppState::get().s_RenderMode == 1)
	{
		...
		DX11QuadDrawer::RenderQuadLinear(pd3dImmediateContext, DX11PhongLighting::GetColorsSRV());
#ifdef STRUCTURE_SENSOR
		if (GlobalAppState::get().s_sensorIdx == GlobalAppState::Sensor_StructureSensor) {
		...
		DX11QuadDrawer::RenderNormal(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_CudaDepthSensor.getNormalMapFloat4(), 4, g_CudaDepthSensor.getColorWidth(), g_CudaDepthSensor.getColorHeight());
	}
		...
		DX11QuadDrawer::RenderNormal(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_rayCast->getRayCastData().d_detailNormalsWorld, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
	}
	else if (GlobalAppState::get().s_RenderMode == 11) {
		DX11QuadDrawer::RenderNormal(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_rayCast->getRayCastData().d_normalsWorld, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
	}
	else if (GlobalAppState::get().s_RenderMode == 12 && !GlobalAppState::get().s_voxelRenderingEnabled) {
	...
	else if (GlobalAppState::get().s_RenderMode == 13) {
		DX11QuadDrawer::RenderQuadDynamic(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_rayCast->getRayCastData().d_rhoD, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
	}
	else if (GlobalAppState::get().s_RenderMode == 14) {
		DX11QuadDrawer::RenderQuadDynamic(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_CudaDepthSensor.getDepthCameraData().d_rhoDData, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
	}
	else if (GlobalAppState::get().s_RenderMode == 15) {
		DX11QuadDrawer::RenderNormal(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_CudaDepthSensor.getDepthCameraData().d_normalData, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
	}
...
void renderToFile(ID3D11DeviceContext* pd3dImmediateContext) {
	...
	const std::string reconstructAlbedoDir = baseFolder + "/" + "reconstruction_albedo/"; if (!util::directoryExists(reconstructAlbedoDir)) util::makeDirectory(reconstructAlbedoDir);
	const std::string reconstructInputAlbedoDir = baseFolder + "/" + "reconstruction_input_albedo/"; if (!util::directoryExists(reconstructInputAlbedoDir)) util::makeDirectory(reconstructInputAlbedoDir);
	const std::string reconstructNormalDir = baseFolder + "/" + "reconstruction_normal/"; if (!util::directoryExists(reconstructNormalDir)) util::makeDirectory(reconstructNormalDir);
	const std::string reconstructOriginDepthNormalDir = baseFolder + "/" + "reconstruction_origin_depth_normal/"; if (!util::directoryExists(reconstructOriginDepthNormalDir)) util::makeDirectory(reconstructOriginDepthNormalDir);
	const std::string reconstructEnhancedDepthNormalDir = baseFolder + "/" + "reconstruction_enhanced_depth_normal/"; if (!util::directoryExists(reconstructEnhancedDepthNormalDir)) util::makeDirectory(reconstructEnhancedDepthNormalDir);
	const std::string reconstructVoxelNormalDir = baseFolder + "/" + "reconstruction_voxel_normal/"; if (!util::directoryExists(reconstructVoxelNormalDir)) util::makeDirectory(reconstructVoxelNormalDir);
	const std::string reconstructLightDir = baseFolder + "/" + "reconstruction_light/"; if (!util::directoryExists(reconstructLightDir)) util::makeDirectory(reconstructLightDir);
	...
	if (GlobalAppState::get().s_voxelRenderingEnabled) {
		...
		g_rayCast->doubleTexRender(g_sceneRep->getHashData(), g_sceneRep->getHashParams(), g_texUpdate->getTexPoolData(), g_texUpdate->getTexPoolParams(), g_CudaDepthSensor.getDepthCameraData(), lastRigidTransform);
	}
	...
	{
		float h_light[9];
		cudaMemcpy(h_light, g_CudaDepthSensor.getDepthCameraData().d_lightData, sizeof(float) * 9, cudaMemcpyDeviceToHost);
		std::ofstream fileOut(reconstructLightDir + ssFrameNumber.str() + ".txt", std::ofstream::out);
		for (int i = 0; i < 9; i++) {
			fileOut << h_light[i] << std::endl;
		}
		fileOut.close();
	}
	mat4f view = mat4f::identity();
	{
		{	// reconstruction color
...

			{
				...
				FreeImageWrapper::saveImage(reconstructAlbedoDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}

			{
				...
				FreeImageWrapper::saveImage(reconstructInputAlbedoDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}

			{
				...
				FreeImageWrapper::saveImage(reconstructNormalDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}
			{
				g_RenderToFileTarget.Clear(pd3dImmediateContext);
				...
				FreeImageWrapper::saveImage(reconstructEnhancedDepthNormalDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}
			{
				g_RenderToFileTarget.Clear(pd3dImmediateContext);
				g_RenderToFileTarget.Bind(pd3dImmediateContext);
				DX11QuadDrawer::RenderNormal(DXUTGetD3D11Device(), pd3dImmediateContext, (float*)g_CudaDepthSensor.getDepthCameraData().d_inputNormalData, 4, g_rayCast->getRayCastParams().m_width, g_rayCast->getRayCastParams().m_height);
				g_CustomRenderTarget.Unbind(pd3dImmediateContext);
				...
				FreeImageWrapper::saveImage(reconstructOriginDepthNormalDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}
			{
				g_RenderToFileTarget.Clear(pd3dImmediateContext);
				...
				FreeImageWrapper::saveImage(reconstructVoxelNormalDir + ssFrameNumber.str() + ".png", image);
				SAFE_DELETE_ARRAY(data);
			}
		}
		...
		bool colored = false;
		if (GlobalAppState::get().s_geometryOptimization && GlobalAppState::get().s_optimizationIdx == 0) {
			DX11PhongLighting::render(DXUTGetD3D11Device(), pd3dImmediateContext, g_CustomRenderTarget.GetSRV(1), (float *)g_rayCast->getRayCastData().d_detailNormals, g_CustomRenderTarget.GetSRV(3), colored, g_CustomRenderTarget.getWidth(), g_CustomRenderTarget.getHeight());
		}
		else {
			DX11PhongLighting::render(pd3dImmediateContext, g_CustomRenderTarget.GetSRV(1), g_CustomRenderTarget.GetSRV(2), g_CustomRenderTarget.GetSRV(3), colored, g_CustomRenderTarget.getWidth(), g_CustomRenderTarget.getHeight());
		}
