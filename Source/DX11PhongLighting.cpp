...
HRESULT DX11PhongLighting::OnD3D11CreateDevice(ID3D11Device* pd3dDevice, unsigned int width, unsigned int height)
{
	...
	descTex.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	descTex.Width = GlobalAppState::get().s_adapterWidth;
	descTex.Height = GlobalAppState::get().s_adapterHeight;
...
void DX11PhongLighting::render(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dDeviceContext, ID3D11ShaderResourceView* positions, float* normals, ID3D11ShaderResourceView* colors, bool useMaterial, unsigned int width, unsigned int height)
{
	...
	pd3dDevice->CreateTexture2D(&descTex, NULL, &s_pLocalTmpTexture);
	pd3dDevice->CreateShaderResourceView(s_pLocalTmpTexture, NULL, &s_pLocalTmpTextureSRV);
	...
	cudaArray* in_array;
	cutilSafeCall(cudaGraphicsMapResources(1, &s_dLocalCuda, 0)); // Map DX texture to Cuda
	cutilSafeCall(cudaGraphicsSubResourceGetMappedArray(&in_array, s_dLocalCuda, 0, 0));
	cudaMemcpyToArray(in_array, 0, 0, normals, 4 * sizeof(float)*width*height, cudaMemcpyDeviceToDevice);
	cutilSafeCall(cudaGraphicsUnmapResources(1, &s_dLocalCuda, 0)); // Unmap DX texture
...
// save render targets
	...
	ID3D11ShaderResourceView* srvs[] = { positions, s_pLocalTmpTextureSRV, colors };
	DX11QuadDrawer::RenderQuad(pd3dDeviceContext, s_PixelShaderPhong, srvs, 3);
