...
extern"C" void texUpdateFromGeometryWithCameraMotionMapCUDA(HashData& hashData, const HashParams& hasParams, TexUpdateData &texUpdateData, TexPoolData& texPoolData, const TexPoolParams& texPoolParams, const RayCastData& rayCastData, const DepthCameraData& cameraData, float *d_motion_map, float *d_occlusionweight);
...
public:
	...
	void bindModelTextures(const RayCastData& rayCastData) {
		bindRenderModelTextures(rayCastData);
	}
	...
	void computeGeometryICPMask(const RayCastData &rayCastData, const DepthCameraData &depthCameraData, const RayCastParams &rayCastParams) {

		computeGeometryICPMaskCUDA(rayCastData, depthCameraData, rayCastParams, m_data.d_sourceMask, m_data.d_targetMask, m_params.m_screen_boundary_width);
#ifdef SHOWMASK
		displayFloatMat(m_data.d_sourceMask, "sourceMask", rayCastParams.m_width, rayCastParams.m_height);
#endif
	...
	void computeDepthMasks(const RayCastData &rayCastData, const DepthCameraData &depthCameraData, const RayCastParams &rayCastParams) {
	...
	void texUpdateFromImageWithCameraMotionMap(HashData& hashData, const HashParams& hashParams,
		...
		numNonUpdatedFrame = 0;
	}

	void texUpdateFromGeometryWithCameraMotionMap(HashData& hashData, const HashParams& hashParams,
		const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, const RayCastData& rayCastData, float *d_motionmap) {
		//texUpdateFromImageWithCameraMotionMapCUDA

		texUpdateFromGeometryWithCameraMotionMapCUDA(hashData, hashParams, m_data, m_texPoolData, m_texPoolParams, rayCastData, depthCameraData, d_motionmap, m_data.d_blendingMask);
	...
	unsigned int getNumNonUpdatedFrame() {
		return numNonUpdatedFrame;
	}
...
private:
	...
	unsigned int numNonUpdatedFrame;
