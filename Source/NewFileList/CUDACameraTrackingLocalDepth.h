...
private:
	...
	void computeBestRigidAlignmentParallel(CameraTrackingDepthLocalInput cameraTrackingInput, Eigen::Matrix3f & intrinsics, unsigned int level, CameraTrackingDepthLocalParameters cameraTrackingParameters, unsigned int maxInnerIter, float condThres, float angleThres, LinearSystemConfidence & conf);

	void renderCorrespondenceOurs(float3 * d_x_rots, float3 * d_x_transs, CameraTrackingDepthLocalInput cameraTrackingInput, unsigned int level, CameraTrackingDepthLocalParameters cameraTrackingParameters, const mat4f & intrinsic, const DepthCameraData & depthCameraData);
	...
	CUDABuildLinearSystemLocalDepth* m_CUDABuildLinearSystem;
