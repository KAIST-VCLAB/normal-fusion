...
#define X_GLOBAL_WARP_STATE_FIELDS \
	...
	X(std::vector<float>, s_cellWidth)\
	X(std::vector<float>, s_cellHeight)\
	X(std::vector<float>, s_offsetx)\
	X(std::vector<float>, s_offsety)\
	X(std::vector<float>, s_sigma)\
	X(std::vector<float>, s_lambdaReg)\
	X(std::vector<int>, s_localWindowHWidth)\
	X(float, s_lambdaNWarp) \
	X(float, s_lambdaCWarp)
...
class GlobalWarpingState
{	
	public:
#define X(type, name) type name;
		X_GLOBAL_WARP_STATE_FIELDS
#undef X
		...
		void setDefault() {
			...
			s_cellWidth.resize(1);
			s_cellHeight.resize(1);
			s_offsetx.resize(1);
			s_offsety.resize(1);
			s_sigma.resize(1);
...
#undef X
		...
		static GlobalWarpingState& getInstance() {
			static GlobalWarpingState s;
			return s;
		}

		static GlobalWarpingState& get() {
			return getInstance();
		}
private:
