...
void CUDARGBDSensor::OnD3D11DestroyDevice()
{
	...
	cutilSafeCall(cudaFree(b_ver));
	cutilSafeCall(cudaFree(b_hor));
	cutilSafeCall(cudaFree(d_f_ver));
	cutilSafeCall(cudaFree(d_b_ver));
	cutilSafeCall(cudaFree(d_f_hor));
	cutilSafeCall(cudaFree(d_b_hor));
	cutilSafeCall(cudaFree(v_ver));
	cutilSafeCall(cudaFree(v_hor));
...
HRESULT CUDARGBDSensor::OnD3D11CreateDevice(ID3D11Device* device, CUDARGBDAdapter* CUDARGBDAdapter)
{
	...
	cutilSafeCall(cudaMalloc(&b_ver, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&b_hor, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&d_f_ver, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&d_b_ver, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&d_f_hor, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&d_b_hor, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&v_ver, sizeof(float) * bufferDimColor));
	cutilSafeCall(cudaMalloc(&v_hor, sizeof(float) * bufferDimColor));

	cutilSafeCall(cudaMalloc(&d_sum, sizeof(float)));
...
HRESULT CUDARGBDSensor::setBlurScore(int numKey, int numPass)
{
	numKeyScoreCounter = numKey;
	maxScoreCounter = numKeyScoreCounter * 2;
	scoreCounter = maxScoreCounter - 1;
	numPassScore = numPass;
	blurScores = (float *)malloc(sizeof(float) * maxScoreCounter);
...
HRESULT CUDARGBDSensor::process(ID3D11DeviceContext* context)
{
	...
	cudaMemcpy(m_depthCameraData.d_rhoDData, m_depthCameraData.d_colorData, sizeof(float4)*m_depthCameraParams.m_imageHeight*m_depthCameraParams.m_imageWidth, cudaMemcpyDeviceToDevice);
	cudaMemcpyArrayToArray(m_depthCameraData.d_rhoDArray, 0, 0, m_depthCameraData.d_colorArray, 0, 0, sizeof(float4)*m_depthCameraParams.m_imageHeight*m_depthCameraParams.m_imageWidth, cudaMemcpyDeviceToDevice);

	// Check the blurness
	if (true) {
		unsigned int kernelSize = 9;

		lowPassFilterVerticalFloatMap(b_ver, d_intensityMapFilteredFloat, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), kernelSize);
		lowPassFilterHorizontalFloatMap(b_hor, d_intensityMapFilteredFloat, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), kernelSize);

		float s_f_ver = computeAndSumDifferenceMap(d_f_ver, d_sum, d_intensityMapFilteredFloat, d_intensityMapFilteredFloat, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), 0, -1);
		computeDifferenceMap(d_b_ver, b_ver, b_ver, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), 0, -1);

		float s_f_hor = computeAndSumDifferenceMap(d_f_hor, d_sum, d_intensityMapFilteredFloat, d_intensityMapFilteredFloat, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), -1, 0);
		computeDifferenceMap(d_b_hor, b_hor, b_hor, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), -1, 0);

		float s_v_ver = computeAndSumDifferenceMap(v_ver, d_sum, d_f_ver, d_b_ver, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), 0, 0);
		float s_v_hor = computeAndSumDifferenceMap(v_ver, d_sum, d_f_hor, d_b_hor, m_RGBDAdapter->getWidth(), m_RGBDAdapter->getHeight(), 0, 0);
		
		float b_f_ver = (s_f_ver - s_v_ver) / s_f_ver;
		float b_f_hor = (s_f_hor - s_v_hor) / s_f_hor;

		// 0.0 = best, 1.0 = worst
		appendScore(std::max(b_f_ver, b_f_hor));
	}
