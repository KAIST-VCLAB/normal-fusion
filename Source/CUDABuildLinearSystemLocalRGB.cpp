...
void CUDABuildLinearSystemLocalRGB::applyBLs4(CameraTrackingLocalInput cameraTrackingInput, Eigen::Matrix3f& intrinsics, CameraTrackingLocalParameters cameraTrackingParameters, float3 *d_x_rot, float3 *d_x_trans, float3 *d_x_step_rot, float3 *d_x_step_trans, float lambdaReg, float *d_system, unsigned int level, LinearSystemConfidence& conf)
{

	Eigen::Matrix3f intrinsicsRowMajor = intrinsics.transpose(); // Eigen is col major / cuda is row major
	computeNormal3EquationsAllRegLocal(cameraTrackingParameters.imageWidth, cameraTrackingParameters.imageHeight, d_system, d_temp, d_x_rot, d_x_trans, d_x_step_rot, d_x_step_trans, lambdaReg, cameraTrackingInput, intrinsicsRowMajor.data(), cameraTrackingParameters);

}

void CUDABuildLinearSystemLocalRGB::applyBLsFullTexture(CameraTrackingLocalInput cameraTrackingInput, Eigen::Matrix3f& intrinsics, CameraTrackingLocalParameters cameraTrackingParameters, float3 *d_x_rot, float3 *d_x_trans, float3 *d_x_step_rot, float3 *d_x_step_trans, float lambdaReg, float *d_system, unsigned int level, LinearSystemConfidence& conf)
{

	Eigen::Matrix3f intrinsicsRowMajor = intrinsics.transpose(); // Eigen is col major / cuda is row major
	computeNormal4EquationsAllRegLocal(cameraTrackingParameters.imageWidth, cameraTrackingParameters.imageHeight, d_system, d_temp, d_x_rot, d_x_trans, d_x_step_rot, d_x_step_trans, lambdaReg, cameraTrackingInput, intrinsicsRowMajor.data(), cameraTrackingParameters);
