...
int main(int argc, char** argv)
{
	...
	try {
		...
		std::string fileNameDescGlobalEnhancement;
		...
		if (argc >= 4) {
			...
			fileNameDescGlobalEnhancement = std::string(argv[4]);
		}
		else {
			std::cout << "usage: DepthSensing [fileNameDescGlobalApp] [fileNameDescGlobalTracking] [fileNameDescGlobalWarping] [fileNameDescGlobalEnhancement]" << std::endl;
			...
			fileNameDescGlobalEnhancement = "zParametersEnhancementDefault.txt";
		}
		...
		std::cout << VAR_NAME(fileNameDescGlobalEnhancement) << " = " << fileNameDescGlobalEnhancement << std::endl;
		std::cout << std::endl;
		...
		// Read the global enhancement state
		ParameterFile parameterFileEnhancementWarping(fileNameDescGlobalEnhancement);
		GlobalEnhancementState::getInstance().readMembers(fileNameDescGlobalEnhancement);
		GlobalEnhancementState::getInstance().print();
		{
