...
//__global__ void renderCorrespondenceDepthLocal_kernel(unsigned int imageWidth, unsigned int imageHeight, float *output, CameraTrackingLocalInput cameraTrackingInput, float3x3 intrinsics, CameraTrackingLocalParameters cameraTrackingParameters, float *d_transforms) {
//
...
//
//	renderCorrespondenceDepthLocal_kernel << < grid, block >> > (imageWidth, imageHeight, output, cameraTrackingInput, float3x3(intrinsics), cameraTrackingIParameters, d_transforms);
//
//}
//
__global__ void renderCorrespondenceDepthLocal_kernel(unsigned int imageWidth, unsigned int imageHeight, float4 *output, CameraTrackingDepthLocalInput cameraTrackingInput, float3x3 intrinsics, CameraTrackingDepthLocalParameters cameraTrackingParameters, float3 *d_x_rot, float3 *d_x_trans) {
	...
	if (index.x < imageWidth && index.y < imageHeight) {
		...
		if (pInput.x != MINF && !nInput.x != MINF)
		{
			...
			if (pProjTrans.z > 0.f) {
				...
				float4 targetPos = bilinearInterpolationFloat4(uvModel.x, uvModel.y, cameraTrackingInput.d_targetPos, imageWidth, imageHeight);
				float4 warpedTargetpos = transform.getInverse() * make_float4(make_float3(targetPos), 1.0f);
				output[x] = make_float4(warpedTargetpos.x, warpedTargetpos.y, warpedTargetpos.z, 1.0f);
			}
		}
	}
}
...
extern "C" void renderCorrespondenceDepthLocalCUDA(unsigned int imageWidth, unsigned int imageHeight, float4 *output, CameraTrackingDepthLocalInput cameraTrackingInput, float* intrinsics, CameraTrackingDepthLocalParameters cameraTrackingIParameters, float3* d_x_rot, float3* d_x_trans) {
	...
	renderCorrespondenceDepthLocal_kernel << < grid, block >> > (imageWidth, imageHeight, output, cameraTrackingInput, float3x3(intrinsics), cameraTrackingIParameters, d_x_rot, d_x_trans);

}


__global__ void setDepthTransform_kernel(float3 *d_xOld_rot, float3 *d_xOld_trans, float3 init_rot, float3 init_trans, int nodeN) {
...
extern "C" void setDepthTransform(float3 *d_xold_rot, float3 *d_xold_trans, float3 initrot, float3 inittrans, CameraTrackingDepthLocalParameters cameraTrackingParameters) {
	...
	setDepthTransform_kernel << <blockNodeN, THREAD_PER_BLOCK >> > (d_xold_rot, d_xold_trans, initrot, inittrans, nodeN);

}

__global__ void upsampleDepthGrid_kernel(float3 *d_xold_rot, float3 *d_xold_trans, float3 *d_x_rot, float3 *d_x_trans, float2 offsetCur, float2 offset, float2 cellWHCur, float2 cellWH, int nodeWCur, int nodeW, int nodeHCur, int nodeH) {
...
extern "C" void upsampleDepthGrid(float3 *d_xold_rot, float3 *d_xold_trans, float3 *d_x_rot, float3 *d_x_trans, float2 offsetCur, float2 offset, float2 cellWHCur, float2 cellWH, int nodeWCur, int nodeW, int nodeHCur, int nodeH) {
	...
	upsampleDepthGrid_kernel << <blockNodeN, THREAD_PER_BLOCK >> > (d_xold_rot, d_xold_trans, d_x_rot, d_x_trans, offsetCur, offset, cellWHCur, cellWH, nodeWCur, nodeW, nodeHCur, nodeH);

}



__global__ void updateDepthTransforms_kernel(float3 *d_x_rot, float3 *d_x_trans,  float3 *d_xOld_rot, float3 *d_xOld_trans, float3 *d_xStep_rot, float3 *d_xStep_trans, int nodeN) {
...
extern "C" void updateDepthTransforms(float3 *d_x_rot, float3 *d_x_trans, float3 *d_xOld_rot, float3 *d_xOld_trans, float3 *d_xStep_rot, float3 *d_xStep_trans, int nodeN) {

	const int blockNodeN = (nodeN + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK;

	updateDepthTransforms_kernel << <blockNodeN, THREAD_PER_BLOCK >> > (d_x_rot, d_x_trans, d_xOld_rot, d_xOld_trans, d_xStep_rot, d_xStep_trans, nodeN);

}

__global__ void renderDepthMotionMapKernel( float *d_x_map, float3 *d_x_rot, float3 *d_x_trans, float2 offset, float2 cellWH, int imageWidth, int imageHeight, int nodeWidth, int nodeHeight) {
...
extern"C" void renderMotionMapDepthCUDA(float * d_x_map, float3 *d_x_rot, float3 *d_x_trans, float2 offset, float2 cellWH, int imageWidth, int imageHeight, int nodeWidth, int nodeHeight) {
	...
	renderDepthMotionMapKernel << <grid, block >> > (d_x_map, d_x_rot,d_x_trans, offset, cellWH, imageWidth, imageHeight, nodeWidth, nodeHeight);
