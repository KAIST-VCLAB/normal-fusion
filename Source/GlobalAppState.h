...
//#define WRITE_INPUT_RIGIDCORRECTION %
//#define WRITE_INPUT_NONRIGIDCORRECTION
//#define SHOW_STATUS

#define X_GLOBAL_APP_STATE_FIELDS \
	X(unsigned int, s_sensorIdx) \
	X(bool, s_geometryOptimization )\
	X(unsigned int, s_optimizationIdx) \
	...
	X(bool, s_timingsDetailledEnabledOurs) \
	...
	X(int, s_gain)\
	X(int, s_exposure)\
	X(bool, s_autoWhiteBalanceEnabled)\
	X(bool, s_autoExposureEnabled)\
	X(unsigned int, s_erodeIterOccDepth) \
	X(unsigned int, s_erodeIterStretchGauss) \
	X(unsigned int, s_erodeIterStretchBox) \
	X(unsigned int, s_texKeyFrames) \
	X(int, s_nVideoFrame) \
	X(int, s_startFrame) \
	X(float, s_texAngleThreshold_depth) \
	X(float, s_texAngleThreshold_update) \
	X(float, s_texIntegrationWeightSample) \
	X(float, s_texIntegrationWeightMax) \
	X(bool, s_offlineProcessing)\
	X(bool, s_voxelRenderingEnabled)\
	X(int, s_warpingMode)\
	X(float, s_sigmaDepth) \
	X(float, s_sigmaArea) \
	X(float, s_sigmaAngle) \
	X(int, s_numTextureTileWidth)\
	X(std::string, s_sceneName) \
	X(unsigned int, s_numLightCoefficients) \
	X(int, s_numKeyScoreCounter) \
	X(int, s_numPassScoreCounter) \
	X(bool, s_blurnessRejectionEnable)
...
//s_erodeIter = 20;
//
//s_softErodeMinAlpha = 0.2;
//S_softErodeMaxAlpha = 0.8;
//s_softErodeIter = 10;
//s_softErodeSigma = 2.f;
...
public:
	enum SENSOR_IDX 
	{
		...
		Sensor_SensorDataReaderZhou = 9,
	};
	enum OPTIMIZATION_IDX
	{
		Optim_Ours = 0,
		Optim_Rigid = 1,
		Optim_None = 2,
	...
	std::vector<std::string> export_surfix;
	std::vector<std::string> export_mtlfilename;
...
#undef X
	...
	GlobalAppState() {
		...
		m_pQuery = NULL;
		export_surfix.push_back("_ours");
		export_surfix.push_back("rigid");
		export_surfix.push_back("_none");
	}
