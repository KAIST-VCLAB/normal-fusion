...
//#define __CUDACC__
	...
	__device__
		...
		uint2 uvunormal = make_uint2(uv);
		Texel center = d_texPatches[texture_ind * c_texPoolParams.m_texturePatchWidth * c_texPoolParams.m_texturePatchWidth + uvunormal.y * c_texPoolParams.m_texturePatchWidth + uvunormal.x];
		...
		t.alpha = 0.;
		t.weight = 0;
		t.normal_texture = make_float3(0.0f);

		float val = 0.f;
		float3 colordummy;
		float3 color, colortmp;
		float3 normalTexture, normalTextureTmp;
		float weightf, alpha, normalWeightf;
		weightf = 0.f;
		normalWeightf = 0.0f;
		alpha = 0.f;
		...
		normalTexture = make_float3(0.0f);
		normalTextureTmp = make_float3(0.0f);
		...
		*/
		...
		if (ld.weight >= 0.f) {
			...
			if (ld.normal_texture.x != MINF) {
				normalTextureTmp = ld.normal_texture;
				normalTexture += (1 - weight.x) * (1 - weight.y) * normalTextureTmp;
				normalWeightf += (1 - weight.x) * (1 - weight.y);
			}
		...
		if (rd.weight >= 0.f) {
			...
			if (rd.normal_texture.x != MINF) {
				normalTextureTmp = rd.normal_texture;
				normalTexture += weight.x * (1 - weight.y) * normalTextureTmp;
				normalWeightf += (1 - weight.y) * weight.x;
			}
		...
		if (lu.weight >= 0.f) {
			...
			if (lu.normal_texture.x != MINF) {
				normalTextureTmp = lu.normal_texture;
				normalTexture += (1 - weight.x) * weight.y * normalTextureTmp;
				normalWeightf += (1 - weight.x) * weight.y;
			}
		...
		if (ru.weight >= 0.f) {
			...
			if (ru.normal_texture.x != MINF) {
				normalTextureTmp = ru.normal_texture;
				normalTexture += weight.x * weight.y * normalTextureTmp;
				normalWeightf += weight.x * weight.y;
			}
		...
		t.normal_texture = normalize(normalTexture / normalWeightf);
		t.alpha = alpha / val;
	...
	__device__
		...
		if (texture_ind > 0) {
			...
			texel.normal_texture = make_float3(MINF, MINF, MINF);
			texel.weight = 0.f;
			texel.alpha = -1;
		}
	...
	__device__
		...
		t.normal_texture = make_float3(MINF, MINF, MINF);
		t.alpha = -10.f;
		t.weight = 0.f;
	}
