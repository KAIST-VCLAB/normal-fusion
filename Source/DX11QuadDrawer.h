...
public:
	...
	static HRESULT RenderNormal(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dDeviceContext, float* d_data, unsigned int nChannels, unsigned int width, unsigned int height);
	...
	static void RenderQuadLinear(ID3D11DeviceContext * pd3dDeviceContext, ID3D11ShaderResourceView * srv);
...
private:
	...
	static ID3D11PixelShader*	s_PixelShaderRGBANORMAL;
	static ID3D11PixelShader*	s_PixelShaderRGBALINEAR;
