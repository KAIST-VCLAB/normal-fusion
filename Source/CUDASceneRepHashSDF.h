...
public:
	...
	void integrate(const mat4f& lastRigidTransform, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, unsigned int* d_bitMask, float *d_mask) {
		...
		if (GlobalAppState::get().s_timingsDetailledEnabledOurs) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }
		...
		if (GlobalAppState::get().s_timingsDetailledEnabledOurs) {
			cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop();
			TimingLog::totalTimeIntegrateDepth += m_timer.getElapsedTimeMS(); TimingLog::countTimeIntegrateDepth++;
	...
	void finalizeIntegration(const DepthCameraData& depthCameraData) {
