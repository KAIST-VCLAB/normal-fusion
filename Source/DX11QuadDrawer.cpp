...
ID3D11PixelShader*	DX11QuadDrawer::s_PixelShaderRGBANORMAL = NULL;
ID3D11PixelShader*	DX11QuadDrawer::s_PixelShaderRGBALINEAR = NULL;
...
HRESULT DX11QuadDrawer::OnD3D11CreateDevice( ID3D11Device* pd3dDevice )
{
	...
	V_RETURN(CompileShaderFromFile(L"Shaders/QuadDrawer.hlsl", "QuadRGBANORMAL", "ps_5_0", &pBlob));
	V_RETURN(pd3dDevice->CreatePixelShader(pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &s_PixelShaderRGBANORMAL));
	SAFE_RELEASE(pBlob);

	V_RETURN(CompileShaderFromFile(L"Shaders/QuadDrawer.hlsl", "QuadRGBALINEAR", "ps_5_0", &pBlob));
	V_RETURN(pd3dDevice->CreatePixelShader(pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &s_PixelShaderRGBALINEAR));
	SAFE_RELEASE(pBlob);
...
void DX11QuadDrawer::OnD3D11DestroyDevice()
{
	...
	SAFE_RELEASE(s_PixelShaderRGBANORMAL);
	SAFE_RELEASE(s_PixelShaderRGBALINEAR);
...
HRESULT DX11QuadDrawer::RenderNormal(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dDeviceContext, float* d_data, unsigned int nChannels, unsigned int width, unsigned int height)
{
	return RenderQuadDynamic(pd3dDevice, pd3dDeviceContext, d_data, nChannels, width, height, 1.0f, D3DXVECTOR2(1.0f, 1.0f), DX11QuadDrawer::s_PixelShaderRGBANORMAL);
}
...
void DX11QuadDrawer::RenderQuadLinear(ID3D11DeviceContext* pd3dDeviceContext, ID3D11ShaderResourceView* srv)
{
	RenderQuad(pd3dDeviceContext, srv, 1.0f, D3DXVECTOR2(1.0f, 1.0f), DX11QuadDrawer::s_PixelShaderRGBALINEAR);
}
